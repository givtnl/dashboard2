...
export class BankAccountSignInvitationRejectedGuard implements CanActivate {
  ...
  async canActivate(next: ActivatedRouteSnapshot, state: RouterStateSnapshot): Promise<boolean> {
    ...
    try {
      await this.bankAccountSigningService
        .reject(next.queryParams.organisationId, next.queryParams.accountId, next.queryParams.invitationId)
        .toPromise();
      return true;
    } catch (error) {
      return this.HandleFailure(next);
