...
export class OnboardingOrganisationDetailsSendDataGuard implements CanActivate {
    ...
    async canActivate(next: ActivatedRouteSnapshot, state: RouterStateSnapshot): Promise<boolean> {
        try {
            let organisationId = this.applicationStateService.currentUserModel.organisationId;
            ...
            const toExecuteCalls = providingOrganisations.map(x => this.relationshipService.notify(organisationId, x))
